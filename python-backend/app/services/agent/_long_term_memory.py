...
def _build_session_summary(detail: AgentRunDetailDto, goals: list[str], preferences: list[str], rules: list[str]) -> str:
    lines: list[str] = []
    if goals:
...
async def rebuild_session_memory_cache(
    ...
    *,
    ...
    personal_records = await list_memory_records_for_scope(session, user_id, scope_type="user", scope_id=user_id)

    preferences = [record.content for record in personal_records if record.memory_slot == "preference" and record.content]
...
async def consolidate_agent_memory_events(
    ...
    *,
    ...
    for event in pending_events:
        ...
        identity_lines = [*preferences[:3], *rules[:3]]

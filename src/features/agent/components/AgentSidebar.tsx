...
import { useAgent, useAgentSidebar } from '..';
...
export const AgentSidebar: React.FC = () => {
  ...
  const getSessionPreview = (sessionDto: AgentSessionDto) => {
    const lastRun = sessionDto.runs[sessionDto.runs.length - 1];
    if (!lastRun) return '空会话';
    return lastRun.inputText.slice(0, 40) + (lastRun.inputText.length > 40 ? '...' : '');
  };

  const getSessionStatus = (sessionDto: AgentSessionDto) => {
    const lastRun = sessionDto.runs[sessionDto.runs.length - 1];
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex-1 overflow-auto p-3">
              ...
              {isLoadingHistory ? (
                ...
                <div className="space-y-2">
                  {historySessions.map((sessionDto) => {
                    const status = getSessionStatus(sessionDto);
                    return (
                      <motion.div
                        key={sessionDto.id}
                        initial={{ opacity: 0, y: 5 }}
                        animate={{ opacity: 1, y: 0 }}
                        onClick={() => handleLoadSession(sessionDto.id)}
                      ...
                      >
                        <div className="flex items-start gap-2">
                          ...
                          <div className="flex-1 min-w-0">
                            <p className="text-sm text-text-primary truncate">{sessionDto.title || getSessionPreview(sessionDto)}</p>
                            <div className="flex items-center gap-2 text-xs text-text-muted mt-1">
                              <span>{sessionDto.runs.length} 条对话</span>
                              <span>{formatTime(sessionDto.updatedAt)}</span>
                            </div>
                          </div>

                          <button
                            onClick={(e) => handleDeleteSession(sessionDto.id, e)}

...
import { ProviderModelOption, parseProviderModelOptions } from './agent';

export { parseProviderModelOptions };
...
export async function saveApiConfigs(configs: ApiProvider[]): Promise<void> {
  ...
  for (const provider of configs) {
    ...
    const apiKeyChanged = provider.apiKey ? (existing.apiKey || '') !== provider.apiKey : false;
    const changed =
...
export async function fetchProviderModels(input: {
  baseUrl: string;
  apiKey: string;
  apiFormat: ApiProvider['apiFormat'];
  ...
  for (const endpoint of endpoints) {
    try {
      const fullUrl = new URL(endpoint, input.baseUrl).href;
      const response = await fetch(fullUrl, { headers });
      if (!response.ok) {
        continue;
      }
      const payload = await response.json();
      const models = parseProviderModelOptions(payload, input.apiFormat);
      if (models.length > 0) {
        return models;
      }
    } catch {

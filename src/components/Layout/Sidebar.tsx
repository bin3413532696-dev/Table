...
import { Home, CheckSquare, Calendar, Settings, Wallet, Menu, X, Network, PenSquare } from 'lucide-react';
import { useCurrentUser } from '../../contexts/UserContext';

const menuItems = [
  { path: '/dashboard', icon: Home, label: '首页' },
  { path: '/knowledge', icon: Network, label: '知识库' },
  { path: '/writing', icon: PenSquare, label: '写作' },
            ...
            return (
              ...
              >
                ...
                >
                  ...
                  <span className={`relative z-10 transition-colors duration-200 ${isActive ? 'text-white' : ''}`} style={!isActive ? { color: 'var(--sidebar-text)' } : undefined}>
                    <item.icon size={20} />
                  </span>
                  <span className={`relative z-10 font-medium transition-colors duration-200 ${isActive ? 'text-white' : ''}`} style={!isActive ? { color: 'var(--sidebar-text)' } : undefined}>
                    {item.label}
                  </span>
                  {!isActive && (
                    <motion.div

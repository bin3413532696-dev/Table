...
async function loadKnowledgeSnapshot() {
  const payload = await fetchJson(`${PAGE_ORIGIN}/api/load-data?ts=${Date.now()}`);
  return payload?.data?.knowledge;
}
...
async function saveKnowledgeSnapshot(knowledge) {
  const response = await fetch(`${PAGE_ORIGIN}/api/sync-data`, {
    method: 'POST',
  ...
  return cdp.evaluate(
    toExpression(`
      () => {
        ...
        const providers = JSON.parse(localStorage.getItem('agent_api_configs') || '[]');
...
async function main() {
  ...
  try {
    ...
    const taskBefore = await getPageState(cdp);
    ...
    const createdTask = createTaskResult.after.tasks.find(
      (task) => !taskBefore.tasks.some((beforeTask) => beforeTask.id === task.id) && task.title === taskTitle
    );
    summary.tests.push({
      test: 'create_task',
      toolCalled: createTaskResult.toolCalled,
      persisted: !!createdTask,
    });
    if (!createdTask) {
      summary.failures.push('create_task: 未识别到新建任务');
    }

    if (createdTask) {
      const taskKnowledgeEntityId = `entity:task-${createdTask.id}`;
      ...
      try {
        taskKnowledgeSnapshot = await waitForKnowledgeCondition(
          ...
          {
            description: `task knowledge mapping ${taskKnowledgeEntityId}`,
          }
        );
      } catch (error) {
      ...
      const taskMappedEntity = taskKnowledgeSnapshot?.entities?.find(
        (entity) => entity.id === taskKnowledgeEntityId
      );
    ...
    const financeBefore = await getPageState(cdp);
    ...
    const createdFinance = addFinanceResult.after.finance.find(
      (record) =>
        !financeBefore.finance.some((beforeRecord) => beforeRecord.id === record.id) &&
        record.description === financeDescription
    );
    summary.tests.push({
      test: 'add_finance_record',
      toolCalled: addFinanceResult.toolCalled,
      persisted: !!createdFinance,
    });
    if (!createdFinance) {
      summary.failures.push('add_finance_record: 未识别到新建财务记录');
    }

    if (createdFinance) {
      const financeKnowledgeEntityId = `entity:finance-${createdFinance.id}`;
      ...
      try {
        financeKnowledgeSnapshot = await waitForKnowledgeCondition(
          ...
          {
            description: `finance knowledge mapping ${financeKnowledgeEntityId}`,
          }
        );
      } catch (error) {
        summary.failures.push(
      ...
      const financeMappedEntity = financeKnowledgeSnapshot?.entities?.find(
        (entity) => entity.id === financeKnowledgeEntityId
      );
    ...
    if (createdTask) {
      ...
      const stillExists = deleteTaskResult.after.tasks.some((task) => task.id === createdTask.id);
      ...
      if (!taskKnowledgeRemoved) {
        summary.failures.push('delete_task: 对应任务知识实体未删除');
      }
    }
    ...
    if (createdFinance) {
      ...
      const stillExists = deleteFinanceResult.after.finance.some((record) => record.id === createdFinance.id);
      ...
      if (!financeKnowledgeRemoved) {
        summary.failures.push('delete_finance_record: 对应财务知识实体未删除');
      }
    }

...
async function main() {
  ...
  try {
    ...
    const createTaskPersisted =
      String(afterConfirm.bodyText || '').includes('CDP联调任务') ||
      String(afterConfirm.bodyText || '').includes('工具 create_task 执行结果') ||
      String(afterConfirm.bodyText || '').includes('工具 create_task 已执行成功');

    summary.tests.push({
      test: 'create_task_confirmation',
      toolCalled: afterConfirm.toolBadges.includes(createTaskToolName),
      confirmationVisible,
      persistedAfterConfirm: createTaskPersisted,
    });

    if (!createTaskPersisted) {
      summary.failures.push('create_task_confirmation: 确认后未观察到任务创建成功结果');
    }

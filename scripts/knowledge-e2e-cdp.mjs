...
async function loadDataPayload() {
  const url = `${PAGE_ORIGIN}/api/load-data?ts=${Date.now()}`;
...
async function requireKnowledgeSnapshot() {
  ...
  if (!knowledge || typeof knowledge !== 'object') {
    ...
    throw new Error(
      `Knowledge API unavailable: /api/load-data missing knowledge field (${JSON.stringify(summary)})`
    );
  }
  ...
  await cdp.evaluate(
    toExpression(`
      () => {
        ...
        localStorage.removeItem('knowledge_dataset_cache');
        return true;
      }
    `)
  );
